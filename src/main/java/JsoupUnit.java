...
public class JsoupUnit {
     ...
     * @throws IOException
     */
    public List<String[]> readQueries(File file, boolean stemmed, TYPE type) throws IOException {
        ...
        for (int i = 0; i < keyWords.size(); i += 2) {
            ...
            switch (type) {
                case TPYE_NORMAL:
                    words = keyWords.get(i).text().split("[^\\p{L}\\d]+");
                    break;
                case TYPE_SEMANTIC:
                    words = keyWords.get(i).text().split(", ");
                    break;
                default:
                    words = new String[0];
            }
